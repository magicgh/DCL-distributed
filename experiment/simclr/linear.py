...
def train_val(net, data_loader, train_optimizer, loss_criterion, epoch,
              epochs):
    ...
    with (torch.enable_grad() if is_train else torch.no_grad()):
        for data, target in data_bar:
            ...
            total_correct_1 += torch.sum(
                (prediction[:, 0:1] == target.unsqueeze(dim=-1)).any(
                    dim=-1).float()).item()
            total_correct_5 += torch.sum(
                (prediction[:, 0:5] == target.unsqueeze(dim=-1)).any(
                    dim=-1).float()).item()
...
def run_test():
    ...
    parser.add_argument('--model_path',
                        type=str,
                        default='results/128_0.5_200_512_500_model.pth',
    ...
    model = Net(num_class=len(train_data.classes),
    ...
    for epoch in range(1, epochs + 1):
        train_loss, train_acc_1, train_acc_5 = train_val(
            model, train_loader, optimizer, loss_criterion, epoch, epochs)
        ...
        data_frame.to_csv(f'results/{output_name}_linear_statistics.csv',
                          index_label='epoch')
        if test_acc_1 > best_acc:
            best_acc = test_acc_1
            torch.save(model.state_dict(),
                       f'results/{output_name}_linear_model.pth')